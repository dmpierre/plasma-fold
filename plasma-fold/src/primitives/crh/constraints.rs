...
use crate::datastructures::{
    ...
    transaction::constraints::TransactionVar, user::UserIdVar, utxo::constraints::UTXOVar,
};

use super::{NonceCRH, PublicKeyCRH, TransactionCRH, UserIdCRH, UTXOCRH};
...
pub struct UTXOVarCRH<F: PrimeField> {
    _f: PhantomData<F>,
}

impl<F: PrimeField + Absorb> CRHSchemeGadget<UTXOCRH<F>, F> for UTXOVarCRH<F> {
    ...
    fn evaluate(
        ...
        let input = [input.amount.clone(), input.id.clone()];
        CRHGadget::evaluate(parameters, &input)
    }
}
