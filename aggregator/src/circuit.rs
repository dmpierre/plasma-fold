...
use plasma_fold::{
    ...
    primitives::sparsemt::constraints::{MerkleSparseTreePathVar, MerkleSparseTreeTwoPathsVar},
};
    ...
    FCircuit<C::BaseField> for AggregatorCircuit<C, CVar>
{
    ...
    fn generate_step_constraints(
        ...
        for i in 0..TX_IO_SIZE {
            // TODO: deposit
            let pos = FpVar::new_witness(cs.clone(), || {
                Ok({
                    todo!();
                    C::BaseField::zero()
                })
        ...
        Ok(vec![step + FpVar::one(), tx_root, utxo_root, signer_root])
    }
}
